...
pub mod pyth_solana_receiver {
    ...
    pub fn accept_governance_authority_transfer(
        ...
        // CF-PLANTED: dropped `config.target_governance_authority = None;` here.
        // The two-step transfer is supposed to be atomic — accept must clear the
        // target so the same accept cannot replay against a now-stale target. The
        // `two_step_governance` invariant asserts that after a successful
        // accept_governance_authority_transfer the target_governance_authority is
        // None; without the clear, a fixture-side AcceptGovernanceAuthorityTransfer
        // signed by the original target keypair will succeed twice.
        Ok(())
    }
