// CF-PORT: 1:1 vendor of pythnet/pythnet_sdk/src/messages.rs with the optional
//   strum / quickcheck / borsh-schema feature branches stripped (minimal vendored
//   surface — receiver does not exercise any of those code paths). The
//   solana-program-gated AnchorSerialize/AnchorDeserialize on PriceFeedMessage is
//   preserved verbatim (receiver embeds PriceFeedMessage inside PriceUpdateV2 which
//   is an `#[account]`, so the anchor derives are load-bearing).
