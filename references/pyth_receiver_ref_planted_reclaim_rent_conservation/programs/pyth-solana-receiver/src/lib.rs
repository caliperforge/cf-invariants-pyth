// CF-PORT: 1:1 vendor of target_chains/solana/programs/pyth-solana-receiver/src/lib.rs.
// CF-PORT: upstream `mod calculate_twap_unit_tests` block (lines 687-775 of upstream lib.rs)
//   removed — upstream unit tests removed, not part of the invariant surface.
// CF-PORT: receiver-side solana_program paths preserved verbatim via anchor-lang 1.0.1's
//   re-export — `solana_program::{keccak, program_memory::sol_memcpy,
//   secp256k1_recover::secp256k1_recover}` resolves through `anchor_lang::solana_program`,
//   matching the upstream import shape.
...
use {
    ...
    // CF-PORT: upstream paths through bare `solana_program::*` (keccak, program_memory,
    // secp256k1_recover) are split into modular `solana-keccak-hasher` /
    // `solana-program-memory` / `solana-secp256k1-recover` crates under anchor-lang
    // 1.0.1's 3.x stack. The `as keccak` alias preserves the upstream call shape
    // `keccak::hash(...)`; the other two are named directly.
    solana_keccak_hasher as keccak,
    solana_program_memory::sol_memcpy,
...
pub struct ReclaimRent<'info> {
    #[account(mut)]
    pub payer: Signer<'info>,
    // CF-PLANTED: dropped `constraint = price_update_account.write_authority == payer.key()
    // @ ReceiverError::WrongWriteAuthority` from this attribute. The constraint is the
    // only thing gating the `close = payer` rent reclamation against an arbitrary
    // signer; without it any signer can close any PriceUpdateV2 and pocket the rent.
    // The `reclaim_rent_conservation` invariant asserts that rent only returns to the
    // recorded write_authority — a planted attacker keypair's reclaim_rent call lands
    // the lamports in the attacker, surfacing the conservation violation.
...
fn verify_guardian_signature(
    ...
    let recovered = {
        ...
        // CF-PORT: upstream uses `hashed.0[12..]`; under solana-keccak-hasher 3.x the
        // Hash struct's inner field is private and the canonical accessor is `.to_bytes()`.
        let hashed_bytes = hashed.to_bytes();
        // CF-PORT: solana-program-memory 3.x re-marked sol_memcpy as unsafe (it
        // dereferences raw pointers under the hood). The call is safe here: both
        // slices live inside fixed-size stack arrays we just allocated, the length
        // is fixed at 20, and both arrays are 20-byte aligned. Wrap in `unsafe`
        // and document the invariant.
        unsafe {
            sol_memcpy(&mut eth_pubkey, &hashed_bytes[12..], 20);
        }
