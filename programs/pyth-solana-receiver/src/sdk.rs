// CF-PORT: 1:1 vendor of target_chains/solana/programs/pyth-solana-receiver/src/sdk.rs.
// CF-PORT: upstream `use rand::Rng;` + `pub fn get_random_treasury_id()` removed —
//   the rand helper is an off-chain CLI convenience and is not part of the on-chain
//   compile graph; vendoring rand in the on-chain program would invite its own SBF
//   build issues (rand 0.8 needs platform-tools-specific configuration on Solana SBF).
use {
    ...
    // CF-PORT: anchor 1.0.1 modular shim path (matches lib.rs convention).
